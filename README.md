...
| 階段 | state（產生控制器用的輸入） | 產生並送出的控制器 | error 來源 | 應用的控制器 |
|---|---|---|---|---|
| **step 0 前** | s'、誤差＝0、共振＝0 | C₀ | 無 | 無 |
| **step 0** | s0 | C₁ | 握手傳入的 0（無實際區段） | C₀ → seg₀ |
| **step 1** | s1 | C₂ | seg₀（由 C₀ 產生，剛跑完回傳） | C₁ → seg₁ |
| **step 2** | s2 | C₃ | seg₁（由 C₁ 產生） | C₂ → seg₂ |
| **step 3** | s3 | C₄ | seg₂（由 C₂ 產生） | C₃ → seg₃ |
| **⋮** | ⋮ | ⋮ | ⋮ | ⋮ |
| **step 50** | 無 | 無 | seg₄₉（由 C₄₉ 產生） | C₅₀ → seg₅₀ |
| **step 51** | 無 | 無 | seg₅₀（由 C₅₀ 產生） | 無 |
