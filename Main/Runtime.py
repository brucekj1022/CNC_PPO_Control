...
if use_switch_model:
    ...
    read_file_name  = 'ModelBUE1.pth'   # 單模型

# --- TCP 連線（LabVIEW/cRIO 連進來）---
HOST = "0.0.0.0"
PORT = 5005
#endregion
...
# --- 共振偵測 ---
fft_limit_freq  = 2                 # path_FFT 頻率上限 (Hz)（上機用小值，反應快）
num_low_freq_FC = 3                 # 低頻限制點數量

# --- PPO 超參數（上機不訓練，僅建立網路用）---
class PPO_parameter:
    n_step_learning = 20
    mini_batch = 30
    batch_size = 2000
...
# --- QCQP reward 權重（本檔專用，與 Training/Simulation 刻意不同，勿連動改）---
class CNC_parameter:
    Lq = 10                 # Q 參數階數
